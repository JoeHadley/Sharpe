...
public:
    ...
    void write_out_comparison_vectors(const std::string& indepVar_name, const std::vector<T>& indepVar,const vector<std::string>& depVar_names, const vector<std::vector<T>>& depVars,const vector<std::string>& depVarError_names, const vector<std::vector<T>>& depVarErrors)
    {
        ...
        for (int j = 0 ; j < depVar_names.size(); j++){
            ...
            myfile << "plt.errorbar(" << indepVar_name << "," << depVar_names[j] << "," << depVarError_names[j] << ")" << "\n";
            myfile << "plt.errorbar(" << indepVar_name << "," << depVar_names[j] << "," << depVarError_names[j] << ", label='" << depVar_names[j] << "')" << "\n";
            myfile << "plt.legend()" << "\n";
            myfile << "plt.xlabel(\"Distance (m)\")" << "\n";
            myfile << "plt.ylabel(\"Average hit rate \")" << "\n";
            myfile << "plt.savefig(\"" << trialNumber << "+" << "comparison.png\")" << "\n";
            //myfile << "plt.errorbar(" << indepVar_name << "," << depVar_names[j] << "," << depVarError_names[j] << ", color='blue')" << "\n";
...
int main(){
    ...
    cout << "Optimal angle is" << thetaC << endl;

   
    int trialLength = 5; // Minutes
    cout << "Please enter trial number: ";
    cin >> trialNumber ;
