    ...
    for (int j = 0; j < distances.size(); j++ ){
        ...
        for (int i = 0; i < trialNumber; i++){
           ...
           // cout << "Rifle: " << rifleHitCount << endl;
            ...
            // Assuming doTrial returns a vector of shots, but it's not clear from the provided code
            vector<double> rifleShots = doTrial(rifle, target, trialLength, rifleHitCount, rng);

            // Store the trial results
            rifleTrialHits[i] = rifleHitCount;
            rifleTrialAccuracy[i] = static_cast<double>(rifleHitCount) / static_cast<double>(floor(trialLength * rifle.firingRate));

            // Print the trial results if needed
            cout << "Rifle Trial Hits: " << rifleTrialHits[i] << endl;
            cout << "Rifle Trial Accuracy: " << rifleTrialAccuracy[i] << endl;

            rifleDistanceAccuracies[i] =average(rifleTrialAccuracy);
            rifleDistanceErrors[i]=stand_dev(rifleTrialAccuracy);
